...
作用范围: 已修改文件, 还未添加至暂存区(未执行`git add`)

- 放弃指定文件修改, <文件名>为`.`时表示全部文件

```sh 
git checkout -- <文件名>
```

修改Main.cpp, 未添加到暂存区时撤销修改, Main.cpp恢复至修改前状态

![](/assets/image/20241103_175723.jpg)

#### 修改已暂存

作用范围: 已修改文件, 并添加到暂存区(已执行`git add`)
...
`git reset`只撤回添加操作, 不撤回文件修改, 若想撤回修改还需执行`git checkout`
...
`删除`工作空间改动, 撤销`commit`, 撤销`git add .`
...
`git reset --hard`回退至上次commit后, 暂存区和工作区均会回到上次提交时状态, 所有自上次后修改全被恢复
...
创建仓库时默认创建master(或main)分支, 后续也可手动创建分支
...
创建dev分支并切换, 修改(dev)Main.cpp, 并commit
...
```sh
git remote set-url --delete origin <远程仓库URL>
```
