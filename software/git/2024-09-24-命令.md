...
`<<<<<<<` 表示当前修改, `>>>>>>>` 表示传入修改, 冲突是因为两个分支都修改文件同一位置所导致
...
分别查看master分支与dev分支上main.cpp内容

![](/assets/image/20241207_203425.jpg)

### 选择合并

`git cherry-pick`可选择某个分支单次/多次提交, 并作为新提交引入到当前分支

```sh
git cherry-pick 某次commit_id
```

- 示例, 将dev分支commit "dev-2" 合并到main分支
...
将pick分支 commit "pick 2 update" 合并到master分支, 提示冲突

main.cpp显示如下, 选择保留双方修改, 提交
...
gitGraph
    ...
    switch main
    cherry-pick id: "cherry pick ok"
```

解决冲突后, 提交

后续可使用用`git cherry-pick --continue` 继续合并, 或者`git cherry-pick --abort` 放弃放弃合并
...
- 示例, 以commit 7d4b4bf19eaede7d9c7ef...打标签
...
![](/assets/image/20241207_212510.jpg)
