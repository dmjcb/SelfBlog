...
(1) 需要另一个分支所有变动, 采用`git merge`
(2) 只需要部分变动(某几次提交), 采用`git cherry-pick`

`git cherry-pick`可理解为"挑拣"提交, 它会获取某个分支单次/多次提交，并作为一个新提交引入到当前分支
