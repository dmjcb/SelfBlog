...
回退至上次commit, 修改后文件仍在工作区, 且修改后内容未删除
...
(3) 切换回master分支, 再修改(master)Main.cpp提交
...
若在cherry-pick过程中遇到冲突, git会暂停操作, 并要求手动解决冲突

解决冲突后, 将文件添加到暂存区, 继续cherry-pick
