...
class AutoUploadBlog:
    ...
    _BLOG_IMGUR      = "{0}\\{1}".format(_BLOG, _RESOURCE_IMGUR)
    ...
    def del_unused_images(self):
        ...
        used_imgs = get_used_images(self._BLOG)
        dir_imgs = get_files_ap(self._BLOG_IMGUR)
        # 删除未使用图片
        del_count = 0
        for ap in dir_imgs:
    ...
    def upload_blog(self, msg):
        ...
        if self.auto_git.status(self._BLOG_IMGUR) is False:
            print("更新 Imgur")
            del_all_files(self._IMGUR)
            shutil.copytree(self._BLOG_IMGUR, self._IMGUR, dirs_exist_ok=True)
