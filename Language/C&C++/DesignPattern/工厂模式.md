 ...
 * @LastEditTime : 2024-10-11 22:42:44
-->
...
int main() {
    std::shared_ptr<Product> p = SimpleFactory::CreateProduct('A');
    p->DoUse();
    return 0;
}
...
通过模板参数, Factory 类可以生成任意类型的对象, 而无需修改工厂类的代码

当需要添加新的产品类型时, 只需要定义新的产品类, 并传入到 Factory 类中即可

此模式的扩展性较好, 新产品的添加不会影响现有的代码结构
